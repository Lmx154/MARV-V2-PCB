...
5 V-in-only flight controller for rockets/drones (RP2354B MCU, BMI088/BMP581/ADXL375 sensors, QSPI flash + microSD logging, 8 PWM outputs). Pack-to-5 V conversion is off-board (BEC/BMS). Firmware and PCB layout are not done; the PCB is empty.
...
- `tools/` — `build_power.py` (generator), `check_power_netlist.py`, `run_power_checks.py`.
