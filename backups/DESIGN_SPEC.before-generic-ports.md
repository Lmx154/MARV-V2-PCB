...
| V3V3_RADIO | Switched, current-limited 3.3 V for the external ELRS module (SX1262 + RP2354A); default off; limit 284/321/364 mA min/nom/max |
| V3V3_GNSS, V1V8_MAG | Switched, current-limited 3.3 V for NEO-M9N + BMM350 (234/266/302 mA) with a 1.8 V LDO for the BMM350 core; default off |
...
the protection cutoff) firmware sheds the switched peripheral rails so the
budget falls to 500 mA continuous, which the 3.0 V corner meets with ~90 mV
margin. Shedding order is a firmware decision; MCU, sensors and actuator
outputs keep priority and the radio should be the last rail dropped. On raw
...
Peripheral rails (sheet power_periph, U9/U10/U11, R18-R24, C20-C27, FB1,
J6 radio 8-pin JST-GH, J7 GNSS/MAG 10-pin JST-GH; pending MCU signals on
temporary headers J5 and J8) are two TPS2553DBV switches from V3V3 (its 2.5-6.5 V input
covers 3.3 V; same symbol, footprint and equations as U6): U9 -> V3V3_RADIO
with R_ILIM 82.5k (284/321/364 mA), U10 -> V3V3_GNSS with R_ILIM 100k
(234/266/302 mA). Both EN pins are pulled down and driven by MCU GPIO, so the
rails are **off at power-up**: a shorted cable cannot brown out the MCU, and a
USB-only boot stays inside the USB budget until firmware has checked the
source and granted the loads. FAULT_N of each is an MCU input. The BMM350 core
needs 1.72-1.98 V, so U11 (AP2112K-1.8, from V3V3_GNSS, ~1 mA) feeds V1V8_MAG
on the GNSS connector; VDDIO is 3.3 V, so no level translation. The NEO-M9N
VCC pin gets a ferrite bead and 10 uF locally; V_BCKP is fed from unswitched
V3V3 through 1k so hot-start data survives the rail being switched off.
Connector signal pins (UARTs, I2C, radio busy/dio/reset) are brought to global
labels for the future MCU sheet.
